...
from t2engine.network.t2resnet_multi import *
from t2engine.player.base_player import *
from t2engine.uct.uct_node import *
...
class T2Player_multi(BasePlayer):
    def __init__(self):
        super().__init__()
        # モデルファイルのパス
    ...
    def eval_node(self, board, index):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            y1_wobn, y2_wobn, y1_bn, y2_bn = self.model(x)

            if(self.is_wobn):
                logits = cuda.to_cpu(y1_wobn.data)[0]
                value = cuda.to_cpu(F.sigmoid(y2_wobn).data)[0]
            else:
                logits = cuda.to_cpu(y1_bn.data)[0]
                value = cuda.to_cpu(F.sigmoid(y2_bn).data)[0]
    ...
    def usi(self):
        print('id name T2player')
        print('option name modelfile type string default ' + self.modelfile)
        print('option name is_wobn type check default ' + str('false') )
